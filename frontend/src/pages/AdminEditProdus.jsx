...
export default function AdminEditProdus() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setDataExpirare(
          produs?.dataExpirare
            ? new Date(produs.dataExpirare).toISOString().slice(0, 10)
            : ""
        );
      } catch (e) {
        console.error("Eroare la incarcare produs:", e);
      }
    })();
  }, [id]);
  ...
  async function handleSubmit(e) {
    e.preventDefault();
    try {
      await api.put(`/produse-studio/${id}`, {
        nume,
  ...
  return (
    <div className="max-w-xl mx-auto p-6">
      <h2 className="text-2xl font-bold mb-4">Editeaza Produs (stoc)</h2>
