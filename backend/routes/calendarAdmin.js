...
router.get("/", authRequired, roleCheck("admin", "patiser"), async (req, res) => {
  try {
    ...
    if (!date) {
      return res.status(400).json({ message: "Parametrul 'date' e obligatoriu (YYYY-MM-DD)" });
    }
    ...
    for (const c of comenzi) {
      agenda.push({ hour: c.oraLivrare || c.oraRezervare || "00:00", type: "order", item: c });
    }
    for (const r of rezervari) {
      agenda.push({ hour: (r.timeSlot || "00:00").split("-")[0], type: "booking", item: r });
    }
